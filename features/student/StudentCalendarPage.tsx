...
const StudentCalendarPage = () => {
    const { calendarEvents, timetable } = useAppContext();
    const [currentDate, setCurrentDate] = useState(new Date());

    const combinedEvents = useMemo(() => {
        const academicEvents: (CalendarEvent & { eventSource: 'calendar' })[] = calendarEvents.map(e => ({ ...e, eventSource: 'calendar' }));
        const classEvents: (CalendarEvent & { eventSource: 'timetable', dayOfWeek: string, time: string })[] = timetable.map(t => ({
            id: `tt-${t.day}-${t.time}`,
            eventType: 'Class',
            title: t.subject,
            start: '', 
            end: '',  
            description: `with ${t.faculty} in ${t.room}`,
