...
const Header = ({ user, title, subtitle, onLogout, theme, toggleTheme, onProfileClick }) => (
...
const TimetableGrid = ({ timetable, role = 'student' }) => {
...
const ClassroomManager = ({ classes, students, onSaveEntity, onDeleteEntity, constraints, updateConstraints }) => {
    ...
    const handleChatWindowChange = (e) => updateConstraints({ ...constraints, chatWindow: { ...(constraints.chatWindow || { start: '', end: '' }), [e.target.name]: e.target.value } });
...
const UserForm = ({ onSave, onCancel, availableFaculty, availableStudents }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await onSave({ username: profile.email || '', password, role: role, profileId });
        } catch (err) {
...
const ProfilePictureModal = ({ isOpen, onClose, onSave, currentUser }) => {
    const [imageSrc, setImageSrc] = useState(currentUser.profilePictureUrl || null);
    ...
    const renderContent = () => {
        switch(activeTab) {
            ...
            case 'chat': 
                ...
                return React.createElement(React.Fragment, null,
                    React.createElement("div", { className: "flex gap-4 mb-4" },
                        React.createElement("select", { value: selectedClassId, onChange: (e) => setSelectedClassId(e.target.value), className: "p-2 border rounded-md dark:bg-slate-700 dark:border-slate-600" }, ...teacherClasses.map((c) => React.createElement("option", { key: c.id, value: c.id }, c.name))),
                        React.createElement("select", { value: channel, onChange: (e) => setChannel(e.target.value), className: "p-2 border rounded-md dark:bg-slate-700 dark:border-slate-600" }, React.createElement("option", { value: "query" }, "Query"), React.createElement("option", { value: "attendance" }, "Attendance"))
                    ),
